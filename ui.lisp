...
(defun parse-string (s)
  (let ((from-rank-char (char s 1))
	(from-file-char (char s 0))
	(to-rank-char (char s 3))
	(to-file-char (char s 2))
	(promotion-char (if (= (length s) 5)
		       (char s 4)
		       nil)))
    (let ((from-rank (position from-rank-char "87654321"))
          (from-file (position from-file-char "abcdefgh"))
          (to-rank (position to-rank-char "87654321"))
          (to-file (position to-file-char "abcdefgh"))
      ...
      (let ((from (convert-coordinates from-rank from-file))
          ...
          (vector from to promotion (is-ep-move from to) nil)))))
